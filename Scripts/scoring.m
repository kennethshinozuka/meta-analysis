...
%   is either 'other', 'seed_to_voxel', or 'seed_to_voxel_CIFTI', when the
%   map of functionally connected voxels has a CIFTI filetype.
...
%   datatype is 'seed_to_voxel', where n is the number of Yeo networks that 
...
%   study.
% - num_secondaries: This is the total number of secondary analyses of a
%   the dataset at hand. We normalise the score by this value.
...
function [C_p, C_w, C_p_nosign, C_w_nosign] = scoring(datatype, signs, o1p, o2p, o1w, o2w, C_p, C_w, C_p_nosign, C_w_nosign, num_rois, num_secondaries)
    ...
    if (1 && all(o1p == 0)) | (1 && all(o2p == 0))
        ...
        % Cycle through each pair of networks that ROI #1 or ROI #2
        % overlaps with.
        for ind1 = 1:size(arr_nonzero_o1p,2)
            for ind2 = 1:size(arr_nonzero_o2p,2)
                y1 = arr_nonzero_o1p(2,ind1); % Yeo/Tian network that ROI #1 overlaps with
                y2 = arr_nonzero_o2p(2,ind2); % Yeo/Tian network that ROI #2 overlaps with
                if strcmp(datatype, 'seed_to_voxel')
                    ...
                    score_p = signs(2,s2p) * (arr_nonzero_o1p(1,ind1) + arr_nonzero_o2p(1,ind2))/2/num_rois/(num_secondaries + 1);
                else
                    score_p = signs * (arr_nonzero_o1p(1,ind1) + arr_nonzero_o2p(1,ind2))/2/num_rois/(num_secondaries + 1);
                end
                score_p_nosign = (arr_nonzero_o1p(1,ind1) + arr_nonzero_o2p(1,ind2))/2/num_rois/(num_secondaries + 1);
                C_p(y1,y2) = C_p(y1,y2) + score_p;
                if y1 ~= y2
                    C_p(y2,y1) = C_p(y2,y1) + score_p;
                end
                C_p_nosign(y1,y2) = C_p_nosign(y1,y2) + score_p_nosign;
                if y1 ~= y2
                    C_p_nosign(y2,y1) = C_p_nosign(y2,y1) + score_p_nosign;   
                end
            end
        end
    end
    ...
    if o1w == 0 | o2w == 0
        ...
        if strcmp(datatype, 'seed_to_voxel') | strcmp(datatype, 'seed_to_voxel_CIFTI')
            ...
            score_w = signs(2,s2w) * 1/num_rois/(num_secondaries + 1);
        else
            score_w = signs * 1/num_rois/(num_secondaries + 1);
        end
        score_w_nosign = 1/num_rois/(num_secondaries + 1);
        C_w(o1w,o2w) = C_w(o1w,o2w) + score_w;
        if o1w ~= o2w
            C_w(o2w,o1w) = C_w(o2w,o1w) + score_w;
        end
        C_w_nosign(o1w,o2w) = C_w_nosign(o1w,o2w) + score_w_nosign;
        if o1w ~= o2w
            C_w_nosign(o2w,o1w) = C_w_nosign(o2w,o1w) + score_w_nosign;
        end
    end
